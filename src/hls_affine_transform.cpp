...
#pragma HLS PIPELINE II=MM_PPC_/STRM_OUT_PPC_
          ...
          loopBlockColsPpc: for(auto II_=0;II_<(MM_PPC_/STRM_OUT_PPC_);++II_){
            ...
            loopStrmOutPpc: for(auto T_=0;T_<STRM_OUT_PPC_;++T_){
...
#pragma HLS LOOP_TRIPCOUNT min=MAX_COLS_/BLOCK_SIZE_ max=MAX_COLS_/BLOCK_SIZE_
      ...
      bool topLeftXSet_[STRM_INTR_PPC_];
      loopInitTopLeftX: for(auto I_=0;I_<STRM_INTR_PPC_;++I_){
#pragma HLS UNROLL
        topLeftXSet_[I_]=false;
      }

      bool topLeftYSet_[STRM_INTR_PPC_];
      loopInitTopLeftY: for(auto I_=0;I_<STRM_INTR_PPC_;++I_){
#pragma HLS UNROLL
        topLeftYSet_[I_]=false;
      }

      ap_int<16> topLeftX_[STRM_INTR_PPC_];
      ap_int<16> topLeftY_[STRM_INTR_PPC_];
...
#pragma HLS PIPELINE II=1
          ...
          loopStrmIntrPpc: for(auto II_=0;II_<STRM_INTR_PPC_;++II_){
            ...
            if(!topLeftXSet_[II_]){
              topLeftXSet_[II_]=true;
              topLeftX_[II_]=ap_int<16> {SrcX_(II_*16+15,II_*16)};
            } else if(topLeftX_[II_]>ap_int<16> {SrcX_(II_*16+15,II_*16)}){
              topLeftX_[II_]=ap_int<16> {SrcX_(II_*16+15,II_*16)};
            }
            if(!topLeftYSet_[II_]){
              topLeftYSet_[II_]=true;
              topLeftY_[II_]=ap_uint<16> {SrcY_(II_*16+15,II_*16)};
            } else if(topLeftY_[II_]>ap_int<16> {SrcY_(II_*16+15,II_*16)}){
              topLeftY_[II_]=ap_uint<16> {SrcY_(II_*16+15,II_*16)};
            }
          }
      ...
      auto topLeftX2_ {topLeftX_[0]};
      auto topLeftY2_ {topLeftY_[0]};
      loopTopLeftMax: for(auto II_=0;II_<STRM_INTR_PPC_;++II_){
#pragma HLS PIPELINE II=1
        if(topLeftX2_>topLeftX_[II_]){
          topLeftX2_=topLeftX_[II_];
        }
        if(topLeftY2_>topLeftY_[II_]){
          topLeftY2_=topLeftY_[II_];
        }
      }

      topLeftX.write(topLeftX2_);
      topLeftY.write(topLeftY2_);
    }
  }
}

template<int PERFORMANCE_FACTOR_,int SRCAXI_DEPTH_,int CHANNELS_,int DEPTH_,int MM_PPC_,int MAX_STRIDE_,int MAX_ROWS_,int STRM_INTR_PPC_,int MAX_COLS_,int BLOCK_SIZE_,int ROWS_MARGIN_,int COLS_MARGIN_>
static void Func2(
...
#pragma HLS LOOP_TRIPCOUNT min=MAX_COLS_/BLOCK_SIZE_ max=MAX_COLS_/BLOCK_SIZE_
      ...
      static ap_uint<Axi_Vid_Bus_Width<CHANNELS_,DEPTH_,1>::Value> tmp_[PERFORMANCE_FACTOR_][(2*BLOCK_SIZE_)][(2*BLOCK_SIZE_)];
#pragma HLS ARRAY_PARTITION variable=tmp_ type=block factor=PERFORMANCE_FACTOR_ dim=1

#pragma HLS ARRAY_PARTITION variable=tmp_[0] type=cyclic factor=8 dim=2
#pragma HLS BIND_STORAGE variable=tmp_[0] type=RAM_T2P impl=URAM latency=3

#if 1<D_PERFORMANCE_FACTOR_
#pragma HLS ARRAY_PARTITION variable=tmp_[1] type=cyclic factor=8 dim=2
#pragma HLS BIND_STORAGE variable=tmp_[1] type=RAM_T2P impl=URAM latency=3

#if 2<D_PERFORMANCE_FACTOR_
#pragma HLS ARRAY_PARTITION variable=tmp_[2] type=cyclic factor=8 dim=2
#pragma HLS BIND_STORAGE variable=tmp_[2] type=RAM_T2P impl=URAM latency=3

#pragma HLS ARRAY_PARTITION variable=tmp_[3] type=cyclic factor=8 dim=2
#pragma HLS BIND_STORAGE variable=tmp_[3] type=RAM_T2P impl=URAM latency=3
#endif
#endif

      loopBlockRows: for(auto JJ_=0;JJ_<(BLOCK_SIZE_+BLOCK_SIZE_/2);++JJ_){
...
#pragma HLS PIPELINE II=1
          ...
          loopBlockColsPpc: for(auto II_=0;II_<MM_PPC_;++II_){
            loopStrmIntrPpc: for(auto TT_=0;TT_<PERFORMANCE_FACTOR_;++TT_){
...
#pragma HLS PIPELINE II=1
          ...
          loopStrmPpc: for(auto II_=0;II_<STRM_INTR_PPC_;++II_){
            if((topLeftY_+ROWS_MARGIN_)>=0&&topLeftY_<MAX_ROWS_&&(topLeftX_+COLS_MARGIN_)>=0&&topLeftX_<MAX_COLS_){
              ...
              srcStreamPix_(II_*CHANNELS_*DEPTH_+CHANNELS_*DEPTH_-1,II_*CHANNELS_*DEPTH_)=tmp_[II_%PERFORMANCE_FACTOR_][index1_][index2_];
            } else {
...
#if 1==D_REDUCED_MASTERS_
#pragma HLS INTERFACE m_axi port=vidWrAxi latency=D_VIDWRAXI_LATENCY_ offset=slave bundle=vidaxibnd num_write_outstanding=D_VIDWRAXI_NUM_WRITE_OUTSTANDING_ max_write_burst_length=16 depth=D_VIDWRAXI_DEPTH_
#pragma HLS INTERFACE m_axi port=affRdAxi latency=D_AFFRDAXI_LATENCY_ offset=slave bundle=affaxibnd num_read_outstanding=D_AFFRDAXI_NUM_READ_OUTSTANDING_   max_read_burst_length=16  depth=D_AFFRDAXI_DEPTH_
#pragma HLS INTERFACE m_axi port=affWrAxi latency=D_AFFWRAXI_LATENCY_ offset=slave bundle=affaxibnd num_write_outstanding=D_AFFWRAXI_NUM_WRITE_OUTSTANDING_ max_write_burst_length=16 depth=D_AFFWRAXI_DEPTH_
#pragma HLS INTERFACE m_axi port=vidRdAxi latency=D_VIDRDAXI_LATENCY_ offset=slave bundle=vidaxibnd num_read_outstanding=D_VIDRDAXI_NUM_READ_OUTSTANDING_   max_read_burst_length=16  depth=D_VIDRDAXI_DEPTH_
#else
...
#pragma HLS STREAM variable=dstBramTrigger_ depth=8
  ...
  //
  Func2<D_PERFORMANCE_FACTOR_,D_AFFRDAXI_DEPTH_,D_MM_CHANNELS_,D_DEPTH_,D_MM_PPC_,D_MAX_STRIDE_,D_MAX_ROWS_,D_STRM_INTR_PPC_,D_MAX_COLS_,D_BLOCK_SIZE_,D_ROWS_MARGIN_,D_COLS_MARGIN_>(
    affRdAxi,
    localStream_,
    padWidth_,
    padHeight_,
